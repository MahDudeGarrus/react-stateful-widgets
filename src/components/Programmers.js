...
const [programersList, setProgrammersList] = useState(listOfAwesome)
const [programmersID, setProgrammersID] = useState()
  ...
  const getNameOfFeatured = () => {
    ...
    // of the program, without needing to inject the information through arguments.
      ...
      for (let i = 0; i < programersList.length; i++) {
        if (programersList[i].id === programmersID) {
          return programersList[i].name
        }
      }
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: `${programmersID ? 'gold' : 'royalblue'}`
  };
